...
static bool IsSafePfsComponent(std::string_view name) {
...
#ifdef PS4FFPSC_STANDALONE
                    // A standalone caller owns the staging layout.  Keeping the supplied
                    // root also makes the containment boundary explicit.
        ...
        if (dinode_reached) {
            for (int j = 0; j < 0x10000; j += ent_size) { // Skip the first parent and child.
...
#ifdef PS4FFPSC_STANDALONE
                if (dirent.namelen < 0 || dirent.namelen > sizeof(dirent.name)) {
                    failreason = "Unsafe PFS directory entry length";
                    return false;
                }
#endif
                ...
                if ((table.type == PFS_FILE || table.type == PFS_DIR) &&
                    !IsSafePfsComponent(table.name)) {
                    failreason = "Unsafe PFS path component";
                    return false;
                }
#endif
